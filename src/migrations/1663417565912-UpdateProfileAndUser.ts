...
export class UpdateProfileAndUser1663417565912 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`profile_entity\` DROP COLUMN \`ready\``);
        await queryRunner.query(`ALTER TABLE \`users\` DROP COLUMN \`firstName\``);
        await queryRunner.query(`ALTER TABLE \`users\` DROP COLUMN \`lastName\``);
        await queryRunner.query(`ALTER TABLE \`users\` DROP COLUMN \`isVerified\``);
        await queryRunner.query(`ALTER TABLE \`users\` DROP COLUMN \`acceptedWithCookie\``);
        await queryRunner.query(`ALTER TABLE \`users\` DROP COLUMN \`status\``);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`users\` ADD \`status\` enum ('PASSPORT_UPLOAD', 'MARKS_UPLOAD', 'DIRECTIONS_UPLOAD', 'AWAITING_RESULT', 'GET_RESULT') NOT NULL DEFAULT 'PASSPORT_UPLOAD'`);
        await queryRunner.query(`ALTER TABLE \`users\` ADD \`acceptedWithCookie\` tinyint NULL`);
        ...
        await queryRunner.query(`ALTER TABLE \`profile_entity\` ADD \`ready\` tinyint NOT NULL DEFAULT '0'`);
