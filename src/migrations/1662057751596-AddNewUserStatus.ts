...
export class AddNewUserStatus1662057751596 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`users\` CHANGE \`status\` \`status\` enum ('PASSWORD_UPLOAD', 'MARKS_UPLOAD', 'DIRECTIONS_UPLOAD', 'AWAITING_RESULT', 'GET_RESULT') NOT NULL DEFAULT 'PASSWORD_UPLOAD'`,
    );
  }
